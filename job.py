...
class Job:
    ...
    def run(self):
        ...
        for i in range(self.conf.cycle_num):
            pop.cycle()
            if i % 100 == 0 and len(pop.genomes) + len(pop.nextgen.genomes) > 0:
                logging.info(f"...stage {i:<6} | n {len(pop.genomes):6} | n {len(pop.nextgen.genomes):6}")

        pop.killall()

        pop.record.record(force=True)
