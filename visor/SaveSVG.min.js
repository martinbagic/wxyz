...
(function (global, factory) {
  ...
  async function readCSSFile(file) {
    ...
    getUrl = getUrl.join("/");
    getUrl += "/";
    let fileread = await readFile(getUrl + file)
      .then(function (d) {
        svgStyle.data += d;
      })
      .catch(function (xhr) {});
  ...
  async function readUSE(usefile) {
    let fetchSVG = await d3.xml(usefile).then(function (data) {
      useArr.data = [].map.call(
        data.querySelectorAll("symbol"),
        function (symbol) {
          return {
  ...
  async function save(svgID, cssFile, useFile) {
    ...
    let filename = svgData.getAttribute("filename"); // Martin
    ...
    downloadLink.download = filename == null ? "Figure.svg" : filename;
